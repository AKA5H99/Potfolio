...
(() => {
  ...
  select('.video-container', true).forEach((container) => {
    ...
    // Embedded players provide their own loading state. Never let a delayed
    // third-party iframe leave our decorative loader on screen indefinitely.
    if (media.tagName === 'IFRAME') {
      media.addEventListener('load', hideSpinner, { once: true });
      media.addEventListener('error', hideSpinner, { once: true });
      fallbackTimer = window.setTimeout(hideSpinner, 1500);
      return;
    }

    if (media.tagName === 'VIDEO') {
      ['loadedmetadata', 'loadeddata', 'canplay', 'error', 'abort'].forEach((eventName) => {
        media.addEventListener(eventName, hideSpinner, { once: true });
      });
      if (media.readyState >= HTMLMediaElement.HAVE_METADATA) hideSpinner();
      else fallbackTimer = window.setTimeout(hideSpinner, 8000);
      return;
    }

    media.addEventListener('load', hideSpinner, { once: true });
    media.addEventListener('error', hideSpinner, { once: true });
    if (media.complete) hideSpinner();
    else fallbackTimer = window.setTimeout(hideSpinner, 8000);
  });
